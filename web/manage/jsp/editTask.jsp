        ...
        <div title="纳税人信息" data-options="closable:false" class="basic-info">
            ...
            <table class="kv-table">
                ...
                <tr>
                    ...
                    <td class="kv-content">${requestScope.or2.organName }</td>
                    <td class="kv-label">行业</td>
                    <td class="kv-content">${requestScope.in.industryName }</td>
            ...
            <table class="kv-table">
                ...
                <tr>
                    ...
                    <td class="kv-content"><input type="text" readonly name="taskName" value="${requestScope.s.taskName}"></td>
                    <td class="kv-label">下达部门</td>
                    <td class="kv-content">  
                     <select name="subOrganId" id="selectOrgan">
                            <c:forEach items="${requestScope.organs }" var="organ">
                            	<option value="${organ.id }" ${organ.id == requestScope.or1.id ? "selected":""}>${organ.organName }</option>
                            </c:forEach>                                             
                      </select>
					</td>
                    ...
                    <td class="kv-content">
                     <select name="approverId">
	                      <c:forEach items="${requestScope.taxers }" var="taxer">
                             <option value="${taxer.id }" ${taxer.id == requestScope.taxer1.id ? "selected":""}>${taxer.taxerName }</option>
                          </c:forEach> 
	                    </select>
					</td>
                </tr>
                ...
                <tr>  
                  	...
                  	<td class="kv-content">    	
	                    <select name="executeId">
	                      <c:forEach items="${requestScope.taxers }" var="taxer">
	                            <option value="${taxer.id }" ${taxer.id == requestScope.taxer2.id ? "selected":""}>${taxer.taxerName }</option>
	                         </c:forEach> 
	                    </select>
                 	</td>
                	<td class="kv-label">执行时间</td>
                    <td class="kv-content"><input type="text" value="${requestScope.s.executeTime}"  name="executeTime"></td>
                    
                    <td class="kv-label">任务执行情况</td>
                    <td class="kv-content">
                        <textarea rows="3" name="taskState" style="width: 90%;">${requestScope.s.taskState}</textarea>
                    </td>
...
<script type="text/javascript">
    ...
    $(function(){
    	$("#saveBtn").on("click",function(){
    		...
    		$.post("updateServlet",$("#editTask").serialize(),function(data){
    			if(data.success){
