            ...
            <div title="纳税人信息" data-options="closable:false" class="basic-info">
                ...
                <table class="kv-table" id="addPayer">
                        ...
                        <td class="kv-content">
                            <select name="taxOrganid">
                                <c:forEach items="${requestScope.tax2}" var="tax2">
                                    <option  value="${tax2.id}">${tax2.organName}</option>
                                </c:forEach>
                            </select>
                        </td>
                        ...
                        <td class="kv-content">
                            <select name="industryId">
                                <c:forEach items="${requestScope.tax1}" var="tax1">
                                    <option  value="${tax1.id}">${tax1.industryName}</option>
                                </c:forEach>
                            </select>
                        </td>
                    </tr>
                        ...
                        <td class="kv-content">
                            <select name="invoiceType">
                                <c:forEach items="${requestScope.tax3}" var="tax3">
                                    <option value="${tax3.id}">${tax3.invoiceType}</option>
                                </c:forEach>
                            </select>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="kv-content">
                            <select name="userId">
                                <c:forEach items="${requestScope.off}" var="off">
                                    <option value="${off.id}">${off.taxerName}</option>
                                </c:forEach>
                            </select>
                        </td>
                        <td class="kv-label">当前时间</td>
                        <td class="kv-content" value="2017-01-20">${requestScope.date } </td>
                    </tr>
                    </tbody>
  ...
  <script type="text/javascript">
	//添加纳税人
		$("#savePayer").on("click",function(){
			...
			$.post("InsertServlet",$("#addForm").serialize(),function(data){
				if(data.success){
